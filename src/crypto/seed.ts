// @ts-nocheck
// ============================================================================
// SEED Block Cipher — RFC 4269 (128-bit block, 128-bit key)
// ============================================================================
...
function sub32(a, b) { return (a - b + 0x100000000) >>> 0; }
...
/*
  ...
  Z (= Z3 || Z2 || Z1 || Z0) of the function G 
  with four 8-bit inputs X (= X3 || X2 || X1 || X0) are as follows:
*/
function G(x) {
  // X = X3 || X2 || X1 || X0
  return (SS3[(x >>> 24) & 0xFF] ^ SS2[(x >>> 16) & 0xFF] ^ SS1[(x >>> 8) & 0xFF] ^ SS0[x & 0xFF]) >>> 0;
}

function keySchedule(keyBuf) {
  let K0 = r32(keyBuf, 0), K1 = r32(keyBuf, 4);
  let K2 = r32(keyBuf, 8), K3 = r32(keyBuf, 12);
  const rk = new Uint32Array(32);
  for (let i = 0; i < 16; i++) {
    rk[i * 2] = G(add32(K0, sub32(K2, KC[i])));
    rk[i * 2 + 1] = G(sub32(add32(K1, KC[i]), K3));
    if ((i + 1) % 2 === 1) {
      const t = K0 & 0xFF;
      K0 = ((K0 >>> 8) | ((K1 & 0xFF) << 24)) >>> 0;
      K1 = ((K1 >>> 8) | (t << 24)) >>> 0;
    } else {
      const t = (K2 >>> 24) & 0xFF;
      K2 = ((K2 << 8) | ((K3 >>> 24) & 0xFF)) >>> 0;
      K3 = ((K3 << 8) | t) >>> 0;
    }
  }
  return rk;
...
function F(R0, R1, Ki0, Ki1) {
  const A = (R0 ^ Ki0) >>> 0;
  const B = (R1 ^ Ki1) >>> 0;
  const G1 = G((A ^ B) >>> 0);
  const G2 = G(add32(A, G1));
  const G3 = G(add32(G2, G1));
  return [add32(G3, G2), G3];
}


function encryptBlock(block, rk) {
  let L0 = r32(block, 0), L1 = r32(block, 4);
  let R0 = r32(block, 8), R1 = r32(block, 12);
  for (let i = 0; i < 15; i++) {
    const [f0, f1] = F(R0, R1, rk[i * 2], rk[i * 2 + 1]);
    const T0 = R0, T1 = R1;
    R0 = (L0 ^ f0) >>> 0; R1 = (L1 ^ f1) >>> 0;
    L0 = T0; L1 = T1;
  }
  const [f0, f1] = F(R0, R1, rk[30], rk[31]);
  L0 = (L0 ^ f0) >>> 0; L1 = (L1 ^ f1) >>> 0;
  const out = Buffer.alloc(16);
  w32(out, 0, L0); w32(out, 4, L1);
  w32(out, 8, R0); w32(out, 12, R1);
  return out;
}
...
function decryptBlock(block, rk) {
  let L0 = r32(block, 0), L1 = r32(block, 4);
  let R0 = r32(block, 8), R1 = r32(block, 12);
  const [f0_16, f1_16] = F(R0, R1, rk[30], rk[31]);
  L0 = (L0 ^ f0_16) >>> 0; L1 = (L1 ^ f1_16) >>> 0;
  for (let i = 14; i >= 0; i--) {
    const [f0, f1] = F(L0, L1, rk[i * 2], rk[i * 2 + 1]);
    const T0 = L0, T1 = L1;
    L0 = (R0 ^ f0) >>> 0; L1 = (R1 ^ f1) >>> 0;
    R0 = T0; R1 = T1;
  }
  const out = Buffer.alloc(16);
  w32(out, 0, L0); w32(out, 4, L1);
...
function cipherBlock(block, rk, encrypt) {
  return encrypt ? encryptBlock(block, rk) : decryptBlock(block, rk);
}

function getKey(key) {
  const k = toBuf(key);
  if (k.length !== 16) throw new Error('SEED: key must be 16 bytes');
  return keySchedule(k);
}

function getIV(iv) {
  if (!iv) return Buffer.alloc(16, 0);
  const b = toBuf(iv);
  if (b.length !== 16) throw new Error('SEED: IV must be 16 bytes');
  return b;
}
...
function ecbEncrypt(key, data) {
  ...
  for (let i = 0; i < d.length; i += 16)
    cipherBlock(d.slice(i, i + 16), rk, true).copy(out, i);
  return toHex(out);
}
...
function ecbDecrypt(key, data) {
  ...
  const out = Buffer.alloc(d.length);
  for (let i = 0; i < d.length; i += 16)
    cipherBlock(d.slice(i, i + 16), rk, false).copy(out, i);
  return toHex(out);
}
...
function cbcEncrypt(key, data, iv) {
  ...
  const out = Buffer.alloc(d.length);
  for (let i = 0; i < d.length; i += 16) {
    const blk = d.slice(i, i + 16);
    const xored = Buffer.alloc(16);
    for (let j = 0; j < 16; j++) xored[j] = blk[j] ^ prev[j];
    prev = cipherBlock(xored, rk, true);
    prev.copy(out, i);
  }
  return toHex(out);
}
...
function cbcDecrypt(key, data, iv) {
  ...
  for (let i = 0; i < d.length; i += 16) {
    const blk = d.slice(i, i + 16);
    const dec = cipherBlock(blk, rk, false);
...
function cbcMac(key, data) {
  const rk = getKey(key);
  const d = toBuf(data);
  if (d.length % 16 !== 0) throw new Error('SEED CBC-MAC: data length must be a multiple of 16 bytes');
  let mac = Buffer.alloc(16, 0);
  for (let i = 0; i < d.length; i += 16) {
    const xored = Buffer.alloc(16);
    for (let j = 0; j < 16; j++) xored[j] = d[i + j] ^ mac[j];
    mac = cipherBlock(xored, rk, true);
  }
  return toHex(mac);
}

// F Padding = ISO 9797-1 Method 2: append 0x80 then 0x00 to next 16-byte boundary
...
function fUnpad(hexData) {
  ...
  if (i < 0 || buf[i] !== 0x80) throw new Error('SEED: invalid F padding');
  return toHex(buf.slice(0, i));
}


export { ecbEncrypt, ecbDecrypt, cbcEncrypt, cbcDecrypt, cbcMac };
